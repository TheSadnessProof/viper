...
|---|---------|-------------|-----------|--------|:------:|
| 1 | Board Coordinate & Mailbox Representation | 0..63 index mapping, rank/file math, algebraic notation (e.g. 'e4') | M1 | Spec Miner 2 §2 | DONE |
| 2 | Piece Move Generation: Pawn | Single push, double push, diagonal captures | M1 | R1, AC | DONE |
| 3 | Piece Move Generation: Major/Minor Pieces | Knight jump vectors, Bishop/Rook/Queen ray sliders | M1 | R1, AC | DONE |
| 4 | Piece Move Generation: King | 1-step in 8 directions with boundary checks | M1 | R1, AC | DONE |
| 5 | Special Moves: Castling | Kingside (O-O) & Queenside (O-O-O), move rights tracking, empty squares, transit/landing check prevention | M1 | R1, AC | DONE |
| 6 | Special Moves: En Passant | Target square tracking, double-push trigger, 1-ply expiration, horizontal pin check | M1 | R1, AC | DONE |
| 7 | Special Moves: Pawn Promotion | Auto-detection on rank 8/rank 1, promotion to Q, R, B, N | M1 | R1, AC | DONE |
| 8 | Check Detection & Attack Evaluation | King square attacked by enemy pieces, ray & jump attack detection | M1 | R1, AC | DONE |
| 9 | Strict Legal Move Validation | Filter candidate moves: king never left in check, absolute pin enforcement | M1 | R1, AC | DONE |
| 10 | Terminal States & Draw Evaluation | Checkmate, Stalemate, 50-move rule, Insufficient material dead positions, Threefold repetition | M1 | R1, AC | DONE |
| 11 | State Transitions & History Stack | Immutable `makeMove`, `undoMove`, FEN import/export, SAN move notation | M1 | Spec Miner 2 §9 | DONE |
| 12 | Bot AI: Casual Tier (~900 Elo) | Fast heuristic random legal moves with capture preference | M2 | R3, AC | DONE |
| 13 | Bot AI: Blitz Tier (~1500 Elo) | Minimax depth 3 with alpha-beta pruning and piece-square tables | M2 | R3, AC | DONE |
| 14 | Bot AI: Grandmaster Tier (~2100+ Elo) | Minimax depth 4-5 with alpha-beta, piece-square tables, center control, king safety, MVV-LVA | M2 | R3, AC | DONE |
| 15 | Non-Blocking AI Execution | Web Worker (`chessAi.worker.ts`) with async fallback for 60fps UI | M2 | R3, AC | DONE |
| 16 | Procedural Audio Synthesizer | Web Audio API sound effects for move, capture, check, castling, victory, defeat | M2 | R2, AC | DONE |
| 17 | Obsidian & Crystal Board Aesthetics | Dark luxury glass squares (`slate-800/90` and `slate-200/90`) with cyan neon styling | M2 | R2, AC | DONE |
| 18 | 5 Visual Move Indicators | Cyan destination dots, rose capture rings, crimson check aura, piece selection, last-move trail | M2 | R2, AC | DONE |
| 19 | Zero Text Clutter UI | Purely visual HUD, minimal chrome, coordinate labels (a-h, 1-8) | M2 | R2, AC | DONE |
| 20 | Dual Input Interaction | Unified Pointer Events for click-to-move and drag-and-drop with touch offset | M2 | R2, AC | DONE |
| 21 | Captured Pieces Rack & Material Diff | Trays for captured White/Black pieces with live `+N` differential badge | M2 | R2, AC | DONE |
| 22 | Promotion Selection Modal | Sleek floating dialog over board for Q, R, B, N choice | M2 | R1, AC | DONE |
| 23 | Game Clocks & Timers | 1m, 3m, 5m, 10m, Unlimited clocks with active countdown and flag-fall handling | M2 | R3, AC | DONE |
| 24 | Game Modes & Board Flip | Human vs AI (White/Black/Random) and Pass-and-Play (2 humans with optional 180° flip) | M2 | R3, AC | DONE |
| 25 | Game Over Modal & Celebration | Non-intrusive modal showing outcome with confetti celebration on victory | M2 | R1, R2, AC | DONE |
| 26 | Fullscreen Arena Integration | `ChessArena.tsx` with `GameWindowControls` (Exit, Fullscreen, Minimize, Nav toggle, extraControls) | M2 | R4, AC | DONE |
| 27 | BoardView Playable Wiring | Mark `id: 'chess'` as `isPlayable: true` with launch handler in `BoardView.tsx` | M2 | R4, AC | DONE |
| 28 | App.tsx Arena State Management | Add `'chess'` to `activeArena`, wire `handleLaunchGame('chess')`, fullscreen toggle | M2 | R4, AC | DONE |
| 29 | E2E Opaque-Box Test Suite | Comprehensive test suite covering Tiers 1-4 with Node 24 native test runner | E2E | AC | DONE |
| 30 | Build & Acceptance Verification | Pass 100% E2E tests (243/243 tests pass), clean `npm run build` with 0 errors | Final | AC | DONE |
...
| M3 | Final Acceptance & Adversarial Hardening | Pass 100% E2E & Tier 5 tests (243 tests), adversarial coverage hardening, verify `npm run build` | E2E, M2 | **DONE** |
...
- `tests/chess/tier5_adversarial.test.ts` - White-box adversarial logic and UI tests (DONE)
